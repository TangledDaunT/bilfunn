...
export default function ConfigForm({ config }: { config: any }) {
  ...
  async function save() {
    setBusy(true);
    const payload: any = { duplicatesCount: form.duplicatesCount, cancelKeepsAccess: form.cancelKeepsAccess };
    ...
    setBusy(false);
    setMsg(res.ok ? "Innstillingene er lagret." : "Kunne ikke lagre. Kontroller verdiene.");
  }
  ...
  return (
    <div className="card">
      ...
      <p className="muted small">
        Reglene ligger i databasen, ikke i koden. Endringer gjelder for nye perioder og påvirker ikke allerede betalte
        perioder.
      </p>
      {msg && <div className="note ok" style={{ marginBottom: 12 }}>{msg}</div>}
      <div className="grid g2" style={{ gap: "0 16px" }}>
        {NUMERIC.map(([k, label]) => (
          <div className="field" key={k}>
            <label className="f" htmlFor={k}>{label}</label>
            <input
              className="input"
              id={k}
              inputMode="numeric"
      ...
      <label className="check" style={{ marginBottom: 8 }}>
        <input
          ...
          onChange={(e) => setForm({ ...form, duplicatesCount: e.target.checked })}
        />
      ...
      <label className="check" style={{ marginBottom: 14 }}>
        <input
          ...
          onChange={(e) => setForm({ ...form, cancelKeepsAccess: e.target.checked })}
        />
