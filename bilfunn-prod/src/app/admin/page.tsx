...
export const metadata = pageMetadata(
  "Administrasjon",
  "Skiltnummeret.nos interne administrasjon.",
  "/admin",
  false,
);
...
export default async function AdminPage({
  ...
  const content =
    tab === "kunder" ? (
      ...
      <ConfigForm config={JSON.parse(JSON.stringify(cfg))} />
    ) : (
      await Overview()
    );
...
async function Overview() {
  ...
  const revenue =
    (payments._sum.amountOre ?? 0) - (payments._sum.refundedOre ?? 0);
  const cfg = await getConfig();
  const mrr = active * cfg.renewalPriceOre;
  const count = (n: string) =>
    events.find((e) => e.name === n)?._count.name ?? 0;
  ...
  return (
    <>
      <div className="grid g3" style={{ marginBottom: 14 }}>
        ...
        <Kpi
          v={paywall ? `${Math.round((bought / paywall) * 100)} %` : "–"}
          l="Betalingsmur → kjøp"
        />
          ...
          <table className="data">
            <tbody>
              {funnel.map(([label, n]) => (
                <tr key={label}>
                  <td>{label}</td>
                  <td className="mono-num" style={{ width: 70 }}>
                    {n}
                  </td>
                  <td style={{ width: "60%" }}>
                    <div
          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.id}>
                ...
                <td>
                  {u.subscriptions[0]
                    ? formatDate(u.subscriptions[0].periodEnd)
                    : "–"}
                </td>
          ...
          <tbody>
            {payments.map((p) => (
              <tr key={p.id}>
                ...
                <td>{formatDate(p.createdAt, "nb-NO", true)}</td>
                <td>{p.user.email}</td>
                <td>{p.kind}</td>
                ...
                <td>
                  {p.status === "SUCCEEDED" ? (
                    ...
                    <span className="tag bad">{p.failureCode ?? "feilet"}</span>
                  ) : (
  ...
  return (
    <div className="card">
      ...
      <p className="muted small">
        IP-adresser lagres bare som hash, og kjøretøydata lagres ikke.
      </p>
      <div className="scrollx">
        <table className="data">
          <thead>
          ...
          <tbody>
            {searches.map((s) => (
              <tr key={s.id}>
                ...
                <td>
                  {s.result === "FOUND" ? (
                    <span className="tag ok">FOUND</span>
                  ) : (
  ...
  return (
    <div className="card">
      ...
      {emails.map((e) => (
        <div
          key={e.id}
          style={{ borderBottom: "1px solid var(--line-2)", padding: "10px 0" }}
        >
          <div className="rowsplit">
            <strong>{e.subject}</strong>
            <span className="tiny">
              {formatDate(e.createdAt, "nb-NO", true)}
            </span>
          </div>
          <div className="tiny">
            Til {e.to} · {e.type} ·{" "}
            {e.sentAt
              ? `sendt via ${e.provider}`
              : e.error
                ? `feil: ${e.error}`
                : "kun logget"}
          </div>
      ...
      {!emails.length && (
        <p className="muted small">Ingen e-post sendt ennå.</p>
      )}
    </div>
  );
}
      ...
      {tickets.map((t) => (
        ...
        >
          <div className="rowsplit">
            ...
            <span className="tiny">
              {formatDate(t.createdAt, "nb-NO", true)}
            </span>
          </div>
...
async function Ops() {
  ...
  const avg = recent.length
    ? Math.round(recent.reduce((a, s) => a + s.latencyMs, 0) / recent.length)
    : 0;
    ...
    <>
      ...
      <div className="card">
        ...
        <dl className="spec">
          <dt>Kjøretøydata</dt>
          <dd>
            {status.vehicleApi === "live"
              ? "Statens vegvesen, enkeltoppslag"
              : "Simulert – sett SVV_API_KEY"}
          </dd>
          <dt>Eierdata</dt>
          <dd>
            {status.ownerApi === "live"
              ? "Aktiv"
              : "Krever egen avtale – OWNER_API_KEY er ikke satt"}
          </dd>
          <dt>Mellomlagring</dt>
          <dd>Av</dd>
          <dt>Tidsavbrudd</dt>
      ...
      <div className="card">
        ...
        <div className="scrollx">
          <table className="data">
            ...
            <tbody>
              {audit.map((a) => (
                <tr key={a.id}>
                  <td>{formatDate(a.createdAt, "nb-NO", true)}</td>
                  <td>{a.actor}</td>
                  <td>{a.action}</td>
                </tr>
              ))}
        ...
        {!audit.length && (
          <p className="muted small">Ingen endringer registrert.</p>
        )}
      </div>
    </>
  );
