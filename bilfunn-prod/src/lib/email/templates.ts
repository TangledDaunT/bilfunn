...
<tr><td style="background:#0a2a55;padding:16px 22px;color:#fff;font-weight:700;font-size:16px">Bilfunn</td></tr>
<tr><td style="padding:24px 22px">
<h1 style="margin:0 0 12px;font-size:19px">${title}</h1>${body}
</td></tr>
${footer ? `<tr><td style="padding:16px 22px;background:#f5f8fc;color:#5b6a80;font-size:12px;line-height:1.5">
Bilfunn AS · Storgata 1, 0155 Oslo · support@bilfunn.no<br>
...
const p = (t: string) => `<p style="margin:0 0 12px;font-size:15px;line-height:1.6">${t}</p>`;
...
export function renderEmail(type: EmailType, c: Ctx): { subject: string; html: string; text: string } {
  ...
  switch (type) {
    ...
    case "welcome":
      return mk(
        "Velkommen til Bilfunn",
        layout("Kontoen din er klar", p(`Du har nå tilgang i ${c.days} dager.`) + btn(c.link, "Gå til Min side"))
      );
    case "payment_receipt":
      return mk(
        `Kvittering ${c.receipt} fra Bilfunn`,
        layout(
          "Kvittering",
          p(`Vi har belastet <b>${kr(c.amountOre)}</b>.`) +
            p(`Kvitteringsnummer: ${c.receipt}<br>Dato: ${d(c.at, true)}<br>Herav mva. 25 %: ${kr(c.vatOre)}`) +
            p("Bilfunn AS, org.nr 000 000 000 MVA")
        )
      );
      ...
      return mk(
        ...
        layout(
          ...
          p(`Du har tilgang til <b>${d(c.periodEnd, true)}</b>.`) +
            p(`Deretter fornyes abonnementet automatisk til <b>${kr(c.renewalOre)} per måned</b> inntil du sier opp.`) +
    ...
    case "renewal_reminder":
      return mk(
        ...
        layout(
          ...
          p(`Introduksjonsperioden avsluttes <b>${d(c.periodEnd, true)}</b>, og vi belaster da ${kr(c.renewalOre)}.`) +
            p("Vil du ikke fortsette, kan du si opp selv før dette — det tar to klikk.") +
    ...
    case "renewal_success":
      return mk(
        ...
        layout("Fornyet", p(`Vi har belastet ${kr(c.amountOre)} for perioden fram til ${d(c.periodEnd)}.`))
      );
    case "payment_failed":
      return mk(
        ...
        layout(
          ...
          p(`Vi prøver automatisk igjen. Oppdaterer du betalingsmåten innen <b>${d(c.graceUntil)}</b>, beholder du tilgangen.`) +
    ...
    case "cancellation":
      return mk(
        ...
        layout(
          ...
          p(`Du har tilgang til <b>${d(c.until, true)}</b>. Det blir ingen flere trekk.`)
        )
      );
    case "refund":
      return mk("Refusjon utført", layout("Refusjon", p(`${kr(c.amountOre)} er refundert til betalingsmåten din.`)));
    case "account_deleted":
      return mk(
        "Kontoen er slettet",
        layout(
          "Slettet",
          p("Kontoen, søkehistorikken og kontaktopplysningene dine er slettet. Kvitteringer beholdes i regnskapet så lenge bokføringsloven krever.")
        )
      );
  }
}
...
function mk(subject: string, html: string) {
  return { subject, html, text: html.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim() };
}
